...
*/
    ...
    #define VEC_MULT(a, b) ((v2f){a.x * b.x, a.y * b.y})
    #define VEC_MULT_F(v, f) ((v2f){v.x * f, v.y * f})
    #define VEC_ADD(a, b) ((v2f){a.x + b.x, a.y + b.y})
    #define VEC_SUB(a, b) ((v2f){})
    #define CAP_MIN(v, min) (v < min ? min : v)
    #define CAP_MAX(v, max) (v > max ? max : v)
    #define CAP(v, min, max) (CAP_MAX(CAP_MIN(v, min), max))
    #define VEC_DIST(a, b) (sqrt(pow(b.x - a.x, 2) + pow(b.y - a.y, 2)))
